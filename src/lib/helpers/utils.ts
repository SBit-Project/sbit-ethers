...
export async function addVins(
    ...
    const neededAmountBN = BigNumberEthers.from(new BigNumber(sbitToSatoshi(neededAmount)).toString());
...
function satoshiToSbit(inSatoshi?: BigNumberish): string {
    return shiftBy(inSatoshi || 0, -8);
}

function sbitToSatoshi(inSbit?: BigNumberish): string {
    return shiftBy(inSbit || 0, 8);
}

function checkLostPrecisionInGasPrice(gasPrice: number) {
    const roundedGasPrice = new BigNumber(new BigNumber(satoshiToSbit(gasPrice)).toFixed(8)).toNumber();
    const originalGasPrice = new BigNumber(new BigNumber(satoshiToSbit(gasPrice)).toFixed()).toNumber();
...
export function configureSbitAddressGeneration(hdnode: HDNode): HDNode {
    ...
    // @ts-ignore
    defineReadOnly(hdnode, "sbitAddress", computeAddress(hdnode.publicKey, true));
    return hdnode;
}
...
export function checkTransactionType(tx: TransactionRequest): CheckTransactionType {
    if (!!tx.to === false && (!!tx.value === false || BigNumberEthers.from(tx.value).toNumber() === 0) && !!tx.data === true) {
        const needed = new BigNumber(satoshiToSbit(tx.gasPrice)).times(BigNumberEthers.from(tx.gasLimit).toNumber()).toFixed(8).toString()
        ...
        const needed = !!tx.value === true ? 
            new BigNumber(
                new BigNumber(satoshiToSbit(tx.gasPrice)).toFixed(8))
                    .times(BigNumberEthers.from(tx.gasLimit).toNumber())
                    .plus(satoshiToSbit(tx.value)).toFixed(8) :
            new BigNumber(new BigNumber(satoshiToSbit(tx.gasPrice)).toFixed(8))
        ...
        const gas = new BigNumber(satoshiToSbit(tx.gasPrice)).times(BigNumberEthers.from(tx.gasLimit).toNumber());
        const needed = new BigNumber(satoshiToSbit(tx.value)).plus(gas).toFixed(8);
...
export async function serializeTransactionWith(utxos: Array<any>, fetchUtxos: Function, neededAmount: string, tx: TransactionRequest, transactionType: number, signer: Function, publicKey: string, filterDust: boolean): Promise<string> {
    ...
    let sbitTx: Tx = { version: 2, locktime: 0, vins: [], vouts: [] };
    ...
    if (transactionType === GLOBAL_VARS.CONTRACT_CREATION) {
        ...
        const contractVoutValue = !!tx.value === true ?
            new BigNumber(satoshiToSbit(tx.value)).toNumber() :
            new BigNumber(BigNumberEthers.from("0x0").toNumber() + `e-8`).toFixed(8);
    ...
    for (let i = 0; i < sbitTx.vins.length; i++) {
        if (vinTypes[i].toLowerCase() === "p2pk")  {
            updatedVins.push({ ...sbitTx.vins[i], ['scriptSig']: p2pkScriptSig(await signp2pkhWith(sbitTx, i, signer)) })
        } else {
            updatedVins.push({ ...sbitTx.vins[i], ['scriptSig']: p2pkhScriptSig(await signp2pkhWith(sbitTx, i, signer), publicKey.split("0x")[1]) })
        }
    }
